...
${SUDO} cp -f "${tmp_dir}/${INTEGRATION_SUBDIR}/run_sailpoint.sh" "${SCRIPTS_DIR}/"
${SUDO} chmod +x "${SCRIPTS_DIR}/run_sailpoint.sh"
${SUDO} chown "${INSTALL_USER}:${INSTALL_GROUP}" "${SCRIPTS_DIR}/"*.py "${SCRIPTS_DIR}/requirements.txt" "${SCRIPTS_DIR}/run_sailpoint.sh"
rm -rf "${tmp_dir}"
...
echo ""
echo "  3. Once satisfied, run the full integration (SSH-session safe):"
echo ""
echo "     ${SCRIPTS_DIR}/run_sailpoint.sh"
echo ""
echo "     run_sailpoint.sh launches the process via nohup so your SSH session"
echo "     can safely disconnect or time out without stopping the integration."
echo "     All output is captured to a timestamped log in ${LOGS_DIR}/."
echo ""
echo "     To monitor progress after launch:"
echo "       tail -f ${LOGS_DIR}/sailpoint_run_*.log"
echo ""
